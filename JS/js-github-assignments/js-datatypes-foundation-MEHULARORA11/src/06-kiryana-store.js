...
export function sortByPrice(items, ascending) {
  ...
  let team = Array.from(items)
  if(ascending){
    return team.sort((a,b) => a.price - b.price)
  }else{
    return team.sort((a,b) => b.price - a.price)
  }
}
...
export function formatBill(items) {
  ...
  let a = items.map((item) => `${item.name} x ${item.qty} = Rs.${(item.price*item.qty)}`)
  return a.join('\n')
}
