...
if(typeof(pnrData.pnr) !== 'string' || pnrData.pnr.length !== 10 || /[a-z]/.test(pnrData.pnr)) return null
if(!pnrData.train || typeof(pnrData.train) !== 'object' || Array.isArray(pnrData.train) || pnrData.train === null) return null
...
let passengers = pnrData['passengers'].map((item) => {
  let formattedName = `${item.name.padEnd(20)}(${item.age}/${item.gender})`
  let bookingStatus = item.booking
  let currentStatus = item.current
  let statusLabel
  let isConfirmed = false

  if(item.current === 'CAN'){
    statusLabel = 'CANCELLED'
  }else if(item.current.startsWith('WL')){
    statusLabel = 'WAITING'
  }else if(item.current.startsWith('RAC')){
     statusLabel = 'RAC'
  }else{
  ...
  return {
   formattedName,bookingStatus,currentStatus,statusLabel,isConfirmed // note i have used shorthand property of objects here 
  }
})

let totalPassengers = passengers.length
let confirmed = passengers.filter((item) => item.statusLabel === 'CONFIRMED').length
let waiting = passengers.filter((item) => item.statusLabel === 'WAITING').length
let cancelled = passengers.filter((item) => item.statusLabel === 'CANCELLED').length
let rac = passengers.filter((item) => item.statusLabel === 'RAC').length
let allConfirmed = passengers.every((item) => item.statusLabel === 'CONFIRMED')
let anyWaiting = passengers.some((item) => item.statusLabel === 'WAITING')
// let chartPrepared = passengers.every((item) => item.statusLabel !== 'CANCELLED' && item.isConfirmed) // wrong line as it means that if even the passeger is cancelled then also it will return false as there is every so it will check each one so use differntly 
let nonCancelled = passengers.filter((item) => item.statusLabel !== 'CANCELLED')
let chartPrepared = nonCancelled.every((item) => item.isConfirmed)

let summary = {totalPassengers,confirmed,waiting,cancelled,rac,allConfirmed,anyWaiting}
