...
export function cricketScoreboard(balls) {
  ...
  for(let i=0;i<balls.length;i++){
    // if(/[1-6]/.test(String(balls[i]))){ ==> note this line is dangerous as then 16 is also return true as 6 is there but we don't need 16 so we need to do && String(balls[i]).length === 1
    if(/[1-6]/.test(String(balls[i])) && String(balls[i]).length === 1){ // note for this to work i should be in string 
       totalRuns+=balls[i]
       if(balls[i] === 4){
        fours +=1
       }else if(balls[i] === 6){
  ...
  /**
   * note the line  ==> for (let i = 0; i < balls.length && wickets < 10; i++)
   * */ 
