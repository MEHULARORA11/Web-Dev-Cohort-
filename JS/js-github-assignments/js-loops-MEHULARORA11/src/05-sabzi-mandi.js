...
export function sabziMandiBill(shoppingList, priceList) {
  // Your code here
  let items = shoppingList.filter((item) => priceList.hasOwnProperty(item.name) && priceList[item.name]<=80).map((item) => {
    return {
 name:item.name,
qty:item.qty,
 cost:priceList[item.name]*item.qty
  ...
  if(items.length === 0) return {items:[],totalBill:0}
  let totalBill = items.reduce((acc,curr) => {
 acc+=curr.cost
 return acc
  },0)
