...
export function createSweetItem(name, price, category) {
  // Your code here
  if(!name || (!price && price === 0) || !category || typeof(name) !== 'string' || typeof(category) !== 'string' || typeof(price) !== 'number') return null
...
export function buildMenuBoard(sweets) {
  ...
  menu.id = 'menu-board'
  if(sweets.length === 0) return menu
  sweets.forEach((sweet) => {
    const div  = createSweetItem(sweet.name,sweet.price,sweet.category)
...
export function addSpecialBadge(sweetElement, badgeText) {
  // Your code here
  if(!sweetElement || typeof badgeText !== 'string' || !badgeText) return null
