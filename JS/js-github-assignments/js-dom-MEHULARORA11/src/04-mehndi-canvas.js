...
export function setPatternStyle(element, styles) {
  // Your code here
  if(typeof(styles) !== 'object' || Array.isArray(styles) || styles === null) return 0
  if(!element) return -1
  let count = 0
  for(let sty in styles){
    element.style[sty] = styles[sty] // inportant line the part [sty]
    count++
  }
  return count
}
...
export function animateElement(element, frames) {
  ...
  element.style[Object.keys(frames.at(-1))[0]] = `${Object.values(frames.at(-1))[0]}` 
