...
export function createPandalElement(pandal) {
  // Your code here
  if(!pandal || typeof(pandal.name) !== 'string'
...
export function updatePandalRating(element, newRating) {
  // Your code here
  if(!element || typeof(newRating) !== 'number' || !(newRating >=0 && newRating <=5) || !(element.dataset.rating)) return null
  let oldRating = element.dataset.rating
...
export function filterPandalsByZone(container, zone) {
  ...
  let list = Array.from(container.querySelectorAll('.pandal'))
  let zoneList = list.filter((item) => item.dataset.zone === zone)
  return zoneList
...
export function getPandalsByBudgetRange(container, min, max) {
  // Your code here
  if(!container || typeof(max) !== 'number' || typeof(min) !== 'number') return []

    let list = Array.from(container.querySelectorAll('.pandal'))
  let budgetList = list.filter((item) => item.dataset.budget >=min && item.dataset.budget <=max)
  return budgetList
