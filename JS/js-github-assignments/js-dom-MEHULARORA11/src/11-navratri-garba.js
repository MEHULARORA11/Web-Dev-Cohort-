...
export function insertDancer(stage, newDancer, referenceDancer) {
  // Your code here
  if(!newDancer || !stage) return false
...
export function cloneDancer(dancer, deep) {
  ...
  let copyDancer = dancer.cloneNode(deep)
  if(dancer.id){
...
export function rearrangeStage(stage, order) {
  ...
  let childList = Array.from(stage.children)
 let maxIndice = Math.max(...order)
  if(maxIndice >= childList.length) return false
  ...
  order.forEach((index) => {
    stage.appendChild(childList[index])
  })
  return true
