...
export function setupRemoveButton(button, thaliElement) {
  // Your code here
  if(!thaliElement || !button) return null
  let removeFunct = () => {
    let lastChild = thaliElement.lastElementChild
    if(!lastChild) return 
    lastChild.remove()
  }
  button.addEventListener('click',removeFunct)
  return function cleanup(){
    button.removeEventListener('click',removeFunct)
  }
}
...
export function createThaliManager(thaliElement, counterElement) {
  ...
  let liList = thaliElement.querySelectorAll('li')
  ...
  return {addItem,removeItem,getCount,clear}
}
