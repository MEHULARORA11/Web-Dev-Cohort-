...
export function highlightCheapestChai(document) {
 ...
 const copyElements = Array.from(elements)
 const mutatedElements = copyElements.map((element) => {
  const input = parseInt(element.textContent.slice(1))
   return element
 })

 let lowestPrice = Math.min.apply(null,[...(mutatedElements.map((item) => Number(item.textContent.slice(1))))])
 let lowestPriceElement = mutatedElements.filter((item) => Number(item.textContent.slice(1)) === lowestPrice)[0]
 lowestPriceElement.classList.add('cheapest')

 const dataChai = lowestPriceElement.getAttribute('data-chai')
 elements.forEach((item) => {
  if(item.getAttribute('data-chai') === dataChai){
    item.classList.add('cheapest')
  }else{
