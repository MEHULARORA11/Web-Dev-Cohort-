...
export function renderKiteCard(kite) { 
  ...
  p.className = 'kite-maker'
  p.textContent = `by ${kite.maker}`
...
export function renderGallery(container, kites) {
  // Your code here
  if(!Array.isArray(kites) || !container) return -1

  container.innerHTML = ''
  let count = 0
  kites.forEach((kite) => {
    if(kite !== null){
      let card = renderKiteCard(kite)
      if(card){
...
let filteredKites = kites.filter((kite) => filterFn(kite))
let count = 0
...
order = order || 'asc'
 ...
 return a[sortField].localeCompare(b[sortField])
  ...
  renderGallery(container,copyArr)
  return copyArr
