...
export function findQueueContainer(element) {
  ...
  let element2 = element.closest('.queue-container')
  return element2
}
...
export function getQueuePosition(element) {
  ...
  let list = element.parentNode.children
...
export function getQueueStats(queueContainer) {
  ...
  let waitingList = document.querySelectorAll('.waiting')
  let servingList = document.querySelectorAll('.serving')
  let completedList = document.querySelectorAll('.completed')
  let total = waitingList.length + servingList.length + completedList.length
  return {
    total,
    waiting:waitingList.length,
    serving:servingList.length,
