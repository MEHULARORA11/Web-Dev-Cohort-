...
export async function processPayment(amount, balance, networkStatus) {
  ...
  return new Promise((res,rej) => {
     setTimeout(() => {
        if(typeof(amount) !== 'number' || amount<=0) return rej(new PaymentError("Invalid amount","INVALID_AMOUNT",amount))
      if(amount > balance) return rej(new InsufficientFundsError(amount,balance)) 
        if(networkStatus === 'offline') return rej(new NetworkError(amount))
          if(amount > 100000) return rej(new FraudDetectedError(amount))
            res({
           transactionId: "TXN" + Math.floor(Math.random() * 1000000),
        amount,
        status: "success",
