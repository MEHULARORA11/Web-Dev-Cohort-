...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    for(let resident of this.#residents){
      if(Object.values(resident).includes(name)){
        isValid = true
      }
    }
    if(isValid) return { success: false, message: "Already a resident!" }
  ...
  removeResident(name, passcode) {
    ...
    for(let resident of this.#residents){
      if(Object.values(resident).includes(name)){
           let newArr = this.#residents.filter((item) => item.name !== resident.name)
           this.#residents = newArr.slice()
        isFound = true
        break;
      }
    }
    if(!isFound) return { success: false, message: "Resident nahi mila!" }
