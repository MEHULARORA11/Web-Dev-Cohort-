...
export function prepareOrder(item, prepTime) {
  ...
  return new Promise((res,rej) => {
    ...
    if(typeof(prepTime) !== 'number' || prepTime<=0) return rej(new Error("Invalid prep time!"))
      setTimeout(() => {
...
export function prepareBatch(items) {
  // Your code here
  return new Promise((res,rej) => {
    if(!Array.isArray || items.length === 0) res([])
      let promises = items.map(async (item) => {
           return await prepareOrder(item.name,item.prepTime) 
      })
...
export function getFirstReady(items) {
  // Your code here
  return new Promise((res,rej) => {
    if(items.length === 0) return rej(new Error("No items to prepare!"))
      let promises = items.map(async (item) => {
           return await prepareOrder(item.name,item.prepTime) 
      })
...
export function deliverWithTimeout(orderPromise, timeoutMs) {
  ...
  if(timeoutMs <= 0) return Promise.reject(new Error("Invalid timeout!"))
    return Promise.race([
     orderPromise,new Promise((_,rej) => {
      setTimeout(() => {
...
export async function batchWithRetry(items, maxRetries) {
 ...
 try {
  attempts--
      let response = await prepareBatch(items)
      return response
    } catch (error) {
      if(attempts === 0){
        throw error
