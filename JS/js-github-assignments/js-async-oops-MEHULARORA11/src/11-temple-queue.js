...
let divoteeObj = null
    if(type === 'vip' && this.#vipEnabled) {
      divoteeObj = { name, type, joinedAt: new Date().toISOString() }
      this.#devotees.unshift(divoteeObj)
    }
    if(type === 'vip' && !this.#vipEnabled) {
      divoteeObj = { name, type, joinedAt: new Date().toISOString() }
      this.#devotees.push(divoteeObj)
    }
      if(type === 'regular'){
        divoteeObj = { name, type, joinedAt: new Date().toISOString() }
        this.#devotees.push(divoteeObj)
  ...
  contains(name) {
    // Your code here
    if(this.#devotees.some((item) => item.name === name)){
      return true
    }
    return false
  }
  ...
  static merge(queue1, queue2) {
    // Your code here
    let capacity = queue1.length + queue2.length
    const merged = new TempleQueue(`${queue1.templeName}-${queue2.templeName}`,capacity)
    for (const d of queue1.toArray()) {
  merged.enqueue(d.name, d.type)
}

for (const d of queue2.toArray()) {
  merged.enqueue(d.name, d.type)
} 
return merged
  }
