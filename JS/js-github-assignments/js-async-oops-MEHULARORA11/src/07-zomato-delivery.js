...
export function placeOrder(restaurant, items) {
 ...
 if(typeof(restaurant) !== 'string' || !restaurant || !Array.isArray(items) || items.length === 0) return rej(new Error("Invalid order details!"))
  setTimeout(() => {
    res({
      orderId:Math.floor(Math.random() * 10000),
      restaurant,items,status:'placed',
...
export function confirmOrder(order) {
  // Your code here
  return new Promise((res,rej) => {
    if(typeof order !== 'object' || order === null || !order.hasOwnProperty('orderId') || order.status !== 'placed') return rej(new Error(`Order cannot be confirmed!`))
...
export function assignRider(order) {
  // Your code here
  return new Promise((res,rej) => {
    if(order.status !== 'confirmed') return rej(new Error("Order not confirmed yet!"))
      let riders = ["Rahul", "Priya", "Amit", "Neha", "Vikram"]
    let selectedRider = riders[Math.floor(Math.random() * 5)]
...
export async function deliverOrder(order) {
  // Your code here
  return new Promise((res,rej) => {
    if(order.status !== 'assigned' || !order.rider) return rej(new Error("No rider assigned!"))
      res({ ...order, status: "delivered",deliveredAt: new Date().toISOString()})
  })
}
...
export function processMultipleOrders(orderList) {
  // Your code here
  return new Promise((res,rej) => {
    if(!Array.isArray(orderList) || orderList.length === 0) return rej([])
      const promises = orderList.map(order =>
      processDelivery(order.restaurant, order.items)
    )

    Promise.allSettled(promises).then(results => res(results))
  })
