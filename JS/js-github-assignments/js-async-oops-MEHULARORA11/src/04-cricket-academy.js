...
export class Batsman extends Player {
  ...
  playInnings(runs, balls) {
    ...
    let inningObj = {runs,balls,strikeRate:(runs/balls)*100}
  ...
  getStrikeRate() {
    ...
    let sum = this.innings.reduce((acc,curr) => {
       return acc + curr.strikeRate
    },0)
  ...
  getProfile() {
    // Your code here
    return {...super.getProfile(),battingStyle:this.battingStyle,role: "batsman", totalRuns: this.innings.reduce((acc,curr) => {return acc + curr.runs},0),inningsPlayed: this.innings.length};
  }
}
...
export class Bowler extends Player {
  ...
  bowlSpell(wickets, runsConceded, overs) {
    // Your code here
    if(overs <= 0 || wickets<0 || runsConceded<0) return null
    let economy = runsConceded/overs
    let spellObj = { wickets, runsConceded, overs, economy }
  ...
  getEconomy() {
    ...
    let spellCount = this.spells.reduce((acc,curr) => {return acc + curr.economy},0)
   return spellCount/this.spells.length
  }

  getProfile() {
    // Your code here
    return {...super.getProfile(),bowlingStyle:this.bowlingStyle,role: "bowler",totalWickets:this.spells.reduce((acc,curr) => {return acc + curr.wickets},0),spellsBowled:this.spells.length}
  }
}
...
export class AllRounder extends Player {
  ...
  getProfile() {
    ...
    return {
      ...
      role: "allrounder",
      totalRuns:this.innings.reduce((acc,curr) => {return acc + curr.runs},0),
      totalWickets:this.spells.reduce((acc,curr) => {return acc + curr.wickets},0),
      inningsPlayed:this.innings.length,
      spellsBowled:this.spells.length
    }
  }
