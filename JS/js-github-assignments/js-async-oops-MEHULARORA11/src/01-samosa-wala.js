...
export function createSamosaCart(ownerName, location) {
  ...
  return { 
    ...
    sales: [] ,
    sellItem(itemName, quantity){
      if(!itemName || typeof(itemName) !== 'string' || !this.menu.hasOwnProperty(itemName) || quantity<=0) return -1
      if(this.menu.hasOwnProperty(itemName)){
        let totalPrice = quantity*this.menu[itemName]
     ...
     getDailySales(){
      ...
      return this.sales.reduce((acc,curr) => {
        return acc + curr.total
      },0)
     },
     getPopularItem(){
      if(!this.sales || this.sales.length === 0) return null

      let maxItem = null
      let maxVal = Number.MIN_SAFE_INTEGER 
      this.sales.forEach((item) => {
        if(maxVal<item.total){
          maxVal = item.total
          maxItem = item
        }
      })
...
export function demonstrateThisLoss(cart) {
  // Your code here
  let destructured = cart.sellItem
  return destructured
}

export function fixWithBind(cart) {
  // Your code here
  let bound = cart.sellItem.bind(cart)
  return bound
}
