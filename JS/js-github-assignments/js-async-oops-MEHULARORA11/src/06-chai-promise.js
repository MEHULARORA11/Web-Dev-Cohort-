...
export function orderChai(type, quantity) {
  ...
  return new Promise((res,rej) => {
    ...
    case 'masala':
      break;  
    default: return rej(new Error("Yeh chai available nahi hai!"))
  }

  if(typeof(quantity) !== 'number' || quantity<=0) return rej(new Error("Kitni chai chahiye bhai?"))
    setTimeout(() => {
...
export function checkIngredients(ingredient) {
  // Your code here
  let validIngredient = ["tea", "milk", "sugar", "ginger", "cardamom"]
  return new Promise((res,rej) => {
    if(validIngredient.includes(ingredient)) return res({ ingredient, available: true })
...
export function processChaiQueue(orders) {
  ...
  return new Promise((res) => {
    if(!Array.isArray(orders) || orders.length === 0) return res([])
    ...
    Promise.all(promises).then(res)// doubt
  })
