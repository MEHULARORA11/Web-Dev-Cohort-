...
export class Startup {
  ...
  constructor(name, founder, domain) {
    ...
    switch (domain) {
      ...
      case 'foodtech':        
        break;    
      default: throw new Error("Invalid domain! Choose from: fintech, edtech, healthtech, foodtech")
    }
  }
...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    return new Promise((res,rej) => {
     setTimeout(() => {
           if(!(startup instanceof Startup)) return rej(new Error("Invalid startup!"))
       if (this.#startups.some(item => item.startup.name === startup.name)) return rej(new Error('Startup already admitted!'))
          if(this.#startups.length >= this.maxStartups) return rej(new Error("Incubator full!"))
            this.#startups.push({ startup, admittedAt: new Date().toISOString(), demoCompleted: false })
          res({ success: true, message: `${startup.name} admitted to ${this.name}!` })
     },100)
    })
  }
  ...
  removeStartup(name) {
    // Your code here
    let copy = this.#startups.slice()
    this.#startups = []
    let isRemoved = false
    for(let startup of copy){
        if(startup.startup.name === name) {
          isRemoved = true
          continue
        }
  ...
  async assignMentor(startupName, mentor) {
    ...
    return new Promise((res,rej) => {
     setTimeout(() => {
        if(!(this.#startups.some((item) => item.startup.name === startupName))) return rej(new Error("Startup not found!"))
        this.#mentors.push({ startupName, mentor, assignedAt: new Date().toISOString() })
      res({ success: true, message: `${mentor.name} assigned to ${startupName}` })
     },100)
    ...
    return new Promise((res,rej) => {
      setTimeout(() => {
        ...
        for(let startup of this.#startups){
          if(startup.startup === startupName){
            startup.demoCompleted = true
          }
          arr.push(startup)
        }
        this.#startups = arr
        let feedbackArr = ["Bahut badhiya!", "Accha hai, improve karo", "Investors impressed!"]
        let feedback = feedbackArr[Math.floor(Math.round() * 3)]
        res(
           {
          startup: startupName,
          score: Math.floor(Math.random() * 41) + 60,  // 60-100
  ...
  async batchProcess(startups) {
    // Your code here
     return new Promise((res,rej) => {
      let promises = startups.map((item) => this.admitStartup(item))
  ...
  getStartupsByDomain(domain) {
    // Your code here
    return this.#startups.filter((item) => item.startup.domain === domain).map(item => item.startup)
...
return n > sorted.length ? sorted : sorted.slice(0, n);
  ...
  *[Symbol.iterator]() {
  for (const item of this.#startups) {
    yield item.startup;
  }
}
